...
* **多音字收的是"常用"次要读音**（条目占比 ≥ 7%，见[词库](dictionary.md#单字与多音字)）：
  长=chang、行=hang、乐=yue、重=chong、还=huan 都有了；但 的=di、了=liao、着=zhao、
  给=ji（给予）、和=huo（暖和）这些**故意没收** —— 收了打 `di` 第一个候选就变成「的」。
  要多全，得用带读音标注的字典（pypinyin / Unihan 那种），而不是从词表里数条目。
