...
| 空格 | 有内容就把**选中的候选**提交（`nihao` → 你好；只匹配了前面一段的候选会分段上屏，见下面），否则当普通空格转发给应用 |
...
| 标点 `,` `.` `?` `!` `;` `:` `(` `)` `[` `]` `"` … | 中文模式 + **中文标点**打开时打成全角（`，` `。` `？` `！` …），一步上屏（`nihao,` → 「你好，」）；关掉就按半角处理，其中 `,` `.` 还是翻页键 |
...
| 退格 | 删掉一个字符 |
| `Del` | 删掉**自己拼出来的**那个候选（`user_phrase` 里的）：这串键以后不再出现它，删完候选列表接着显示。词库里原本就有的词删不了（回一句「不是自己拼的，删不了」） |
...
* **分段候选只挑"最长的那一段"**：`nihaoma` 会先给「你好」相关的（吃前 5 个字符），
  再短的（只吃 `ni`）不会一次列出来 —— 想要「你」+「号码」这种切法，先挑「你」那个分段的做法
  得靠整句候选或用户自己记住。
